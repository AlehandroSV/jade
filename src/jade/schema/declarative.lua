...
--- Generate a full model file with CRUD methods
--- @param model table Parsed model from .jade
--- @param all_models table All models for relation resolution
--- @return string Lua code for the model file
function Declarative.generateFullModel(model, all_models)
    local lines = {}
    local name = model.name or "Model"
    local table_name = model.tableName

    lines[#lines + 1] = '-- Auto-generated by Jade ORM'
    lines[#lines + 1] = '-- Source: schema/models.jade'
    lines[#lines + 1] = ''
    lines[#lines + 1] = 'local Jade = require("jade")'
    lines[#lines + 1] = ''
    lines[#lines + 1] = string.format('local %s = Jade.Entity("%s", {', name, table_name)
    ...
    lines[#lines + 1] = ''
    lines[#lines + 1] = string.format('return %s', name)

    return table.concat(lines, '\n')
end

--- Generate all model files from a parsed schema
--- @param schema table Parsed schema with models
--- @return table<string, string> Map of filename to content
function Declarative.generateAllModels(schema)
    local files = {}

    for model_name, model in pairs(schema.models) do
        local filename = model_name .. ".lua"
        files[filename] = Declarative.generateFullModel(model, schema.models)
    end
...
function Declarative.generateMigration(schema, name)
    ...
    lines[#lines + 1] = '-- Generated by Jade ORM'
    lines[#lines + 1] = ''
    lines[#lines + 1] = 'local jade = require("jade")'
    lines[#lines + 1] = ''
