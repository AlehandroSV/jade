describe("Query Builder", function()
    ...
    describe("timeout", function()
        it("has timeout method", function()
            local q = Query.new(User)
            assert.is_function(q.timeout)
        end)

        it("sets timeout in milliseconds", function()
            local q = Query.new(User):timeout(5000)
            assert.are.equal(5000, q._timeout)
        end)

        it("returns self for chaining", function()
            local q = Query.new(User)
            local result = q:timeout(5000)
            assert.are.equal(q, result)
        end)

        it("rejects non-number timeout", function()
            assert.has_error(function()
                Query.new(User):timeout("5000")
            end)
        end)

        it("rejects zero timeout", function()
            assert.has_error(function()
                Query.new(User):timeout(0)
            end)
        end)

        it("rejects negative timeout", function()
            assert.has_error(function()
                Query.new(User):timeout(-100)
            end)
        end)

        it("default timeout is nil", function()
            local q = Query.new(User)
            assert.is_nil(q._timeout)
        end)
    end)
end)
