describe("Encryption", function()
    ...
    describe("validatePath (security)", function()
        ...
        it("rejects Windows absolute paths", function()
            ...
            assert.has_error(function()
                Encryption.validatePath("C:keys.lua", "lua")
            end)
        end)

        it("rejects UNC absolute paths (#181)", function()
            assert.has_error(function()
                Encryption.validatePath("\\\\server\\share\\keys.lua", "lua")
            end)
            assert.has_error(function()
                Encryption.validatePath("//server/share/keys.lua", "lua")
            end)
        ...
        it("uses generic error message to avoid leaking validation details", function()
            ...
            assert.is_truthy(type(err) == "table" and err.code == "J5004" or type(err) == "string")
